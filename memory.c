...
bool memory_region_access_valid(MemoryRegion *mr, hwaddr addr, unsigned size,
	...
	for (i = 0; i < size; i += access_size)
	{
		if (!mr->ops->valid.accepts(mr->opaque,addr + i, access_size, is_write, attrs))
		{
			return false;
		}
	}
...
static MemTxResult memory_region_dispatch_read1(MemoryRegion *mr, hwaddr addr, uint64_t *pval,
...
MemTxResult memory_region_dispatch_read(MemoryRegion *mr, hwaddr addr, uint64_t *pval,
	...
	{
		*pval = unsingned_mem_read(mr,addr,size);
...
static bool memory_region_dispatch_write_eventfds(MemoryRegion *mr, hwaddr addr, uint64_t data,
		                                          unsigned size, MemTxAttrs attrs) {
...
void memory_region_init_ram_nomigrate(MemoryRegion *mr, Object *owner, const char *name,
		                              uint64_t size, Error **errp) {
	memory_region_init_ram_shared_nomigrate(mr, owner, name, size, false, errp);
}

void memory_region_init_ram_shared_nomigrate(MemoryRegion *mr, Object *owner,
		                       const char *name, uint64_t size, bool share, Error **errp) {
	memory_region_init(mr,owner,name,size);
	mr->ram = true;
	mr->terminates = true;
