...
#ifndef QEMU_READLINE_H_
#define QEMU_READLINE_H_

#define READLINE_CMD_BUF_SIZE 4095
#define READLINE_MAX_CMDS 64
#define READLINE_MAX_COMPLETIONS 256

typedef void GCC_FMT_ATTR(2,3) ReadLinePrintFunc(void *opaque, const char *fmt, ...);
typedef void ReadLineFlushFunc(void *opaque);
typedef void ReadLinePrintfFunc(void *opaque);
...
void readline_start(ReadLineState *rs, const char *prompt, int read_password, ReadLineFunc *readline_func, void *readline_opaque);
void readline_restart(ReadLineState *rs);
void readline_show_prompt(ReadLineState *rs);
ReadLineState *readline_init(ReadLinePrintfFunc *printf_func,
                             ReadLineFlushFunc *flush_func,
                             void *opaque, ReadLineCompletionFunc *completion_finder);
void readline_free(ReadLineState *rs);

#endif /* QEMU_READLINE_H_ */
